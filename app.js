(() => {
  ...
  function openPlaceModal(place = null) {
    ...
    const { node, close } = modal(`<h2>${place ? 'Edit place' : 'Add a place'}</h2><p class="intro">Every enabled choice has an equal chance on the wheel.</p><form id="place-form"><div class="field"><label for="place-name">Place or food choice</label><input id="place-name" list="saved-choice-suggestions" maxlength="80" required value="${escapeHtml(place?.name || '')}" placeholder="e.g. Texas Roadhouse, Pizza, Burgers, Cook at Home" autocomplete="off"><datalist id="saved-choice-suggestions">${savedChoices.map(choice => `<option value="${escapeHtml(choice)}">`).join('')}</datalist></div><div class="field"><label for="place-person">Who chose it?</label><select id="place-person" required>${app.state.people.map(person => `<option value="${person.id}" ${person.id === place?.personId ? 'selected' : ''}>${escapeHtml(person.name)}</option>`).join('')}</select></div><p class="error" id="form-error"></p><div class="modal-actions"><button type="button" class="button ghost" data-close-modal>Cancel</button><button class="button primary">${place ? 'Save Changes' : 'Add Place'}</button></div></form>`);
