(() => {
  ...
  function openPersonChoiceSettings() { const rows = app.state.people.map(person => { const choices = app.state.places.filter(place => place.personId === person.id), enabled = choices.filter(choice => choice.enabled).length, isOn = enabled > 0; return `<div class="person-choice-row"><div><strong>${escapeHtml(person.name)}</strong><small>${choices.length ? `${enabled} of ${choices.length} choices on` : 'No choices yet'}</small></div><label class="switch person-choice-switch" aria-label="Turn ${escapeHtml(person.name)}'s choices ${isOn ? 'off' : 'on'}"><input type="checkbox" data-person-choice-toggle="${person.id}" ${isOn ? 'checked' : ''} ${choices.length ? '' : 'disabled'}><span class="slider"></span></label></div>`; }).join('') || '<p class="intro">Add people first, then assign choices to them.</p>'; const { node, close } = modal(`<h2>Person's Choices</h2><p class="intro">Quickly control all choices entered by one person. Individual entries remain separate.</p><div class="person-choice-list">${rows}</div><div class="modal-actions"><button class="button primary" data-close-modal>Done</button></div>`); $$('[data-person-choice-toggle]', node).forEach(input => input.addEventListener('change', async () => { const choices = app.state.places.filter(place => place.personId === input.dataset.personChoiceToggle); choices.forEach(choice => choice.enabled = input.checked); await saveState(); close(); render(); toast(`${personName(input.dataset.personChoiceToggle)}'s choices turned ${input.checked ? 'on' : 'off'}.`); })); }
